// =====================================================================================================================
// rdpp.cpp - Generated by Code_Parser_Cpp - Create Recursive Descent Predictive Parser on 2017-03-12 23:54:34 for C++14
// =====================================================================================================================
#include "rdpp.hpp"
#include "exception.hpp"
#include "lex.hpp"
#include "noad.hpp"

void SkoarParser::fail() {
    toker->dump();
    throw new SkoarParseException(L"Fail");;
}

void SkoarParser::fail_too_deep() {
    toker->dump();
    throw new SkoarParseException(L"Parse tree too deep!");;
}

 SkoarParser::SkoarParser(SkoarToker * tokr) {

    deep = 0;
    toker = tokr;
    
}

 SkoarStats::SkoarStats() {
    tokeFreq[ESkoarToke::AlCoda] = 0.0f;
    tokeFreq[ESkoarToke::AlFine] = 0.0f;
    tokeFreq[ESkoarToke::AlSegno] = 0.0f;
    tokeFreq[ESkoarToke::ArgSpec] = 0.0f;
    tokeFreq[ESkoarToke::AssOp] = 0.0f;
    tokeFreq[ESkoarToke::Bars] = 0.0f;
    tokeFreq[ESkoarToke::BooleanOp] = 0.0f;
    tokeFreq[ESkoarToke::Caesura] = 0.0f;
    tokeFreq[ESkoarToke::Carrot] = 0.0f;
    tokeFreq[ESkoarToke::Choard] = 0.0f;
    tokeFreq[ESkoarToke::Coda] = 0.0f;
    tokeFreq[ESkoarToke::Comment] = 0.0f;
    tokeFreq[ESkoarToke::CondE] = 0.0f;
    tokeFreq[ESkoarToke::CondIf] = 0.0f;
    tokeFreq[ESkoarToke::CondS] = 0.0f;
    tokeFreq[ESkoarToke::Crap] = 0.0f;
    tokeFreq[ESkoarToke::Crotchets] = 0.0f;
    tokeFreq[ESkoarToke::DaCapo] = 0.0f;
    tokeFreq[ESkoarToke::DalSegno] = 0.0f;
    tokeFreq[ESkoarToke::Deref] = 0.0f;
    tokeFreq[ESkoarToke::DynFP] = 0.0f;
    tokeFreq[ESkoarToke::DynForte] = 0.0f;
    tokeFreq[ESkoarToke::DynPiano] = 0.0f;
    tokeFreq[ESkoarToke::DynSFZ] = 0.0f;
    tokeFreq[ESkoarToke::Eighths] = 0.0f;
    tokeFreq[ESkoarToke::Fairy] = 0.0f;
    tokeFreq[ESkoarToke::False] = 0.0f;
    tokeFreq[ESkoarToke::Fine] = 0.0f;
    tokeFreq[ESkoarToke::Float] = 0.0f;
    tokeFreq[ESkoarToke::Freq] = 0.0f;
    tokeFreq[ESkoarToke::HashLevel] = 0.0f;
    tokeFreq[ESkoarToke::Int] = 0.0f;
    tokeFreq[ESkoarToke::LWing] = 0.0f;
    tokeFreq[ESkoarToke::ListE] = 0.0f;
    tokeFreq[ESkoarToke::ListS] = 0.0f;
    tokeFreq[ESkoarToke::ListSep] = 0.0f;
    tokeFreq[ESkoarToke::Loco] = 0.0f;
    tokeFreq[ESkoarToke::LoopE] = 0.0f;
    tokeFreq[ESkoarToke::LoopS] = 0.0f;
    tokeFreq[ESkoarToke::LoopSep] = 0.0f;
    tokeFreq[ESkoarToke::MathOp] = 0.0f;
    tokeFreq[ESkoarToke::Meter] = 0.0f;
    tokeFreq[ESkoarToke::MsgName] = 0.0f;
    tokeFreq[ESkoarToke::MsgNameWithArgs] = 0.0f;
    tokeFreq[ESkoarToke::MsgOp] = 0.0f;
    tokeFreq[ESkoarToke::NamedNoat] = 0.0f;
    tokeFreq[ESkoarToke::Newline] = 0.0f;
    tokeFreq[ESkoarToke::Nosey] = 0.0f;
    tokeFreq[ESkoarToke::OctaveShift] = 0.0f;
    tokeFreq[ESkoarToke::OttavaA] = 0.0f;
    tokeFreq[ESkoarToke::OttavaB] = 0.0f;
    tokeFreq[ESkoarToke::PedalDown] = 0.0f;
    tokeFreq[ESkoarToke::PedalUp] = 0.0f;
    tokeFreq[ESkoarToke::Portamento] = 0.0f;
    tokeFreq[ESkoarToke::Quarters] = 0.0f;
    tokeFreq[ESkoarToke::Quavers] = 0.0f;
    tokeFreq[ESkoarToke::QuindicesimaA] = 0.0f;
    tokeFreq[ESkoarToke::QuindicesimaB] = 0.0f;
    tokeFreq[ESkoarToke::RWing] = 0.0f;
    tokeFreq[ESkoarToke::Rep] = 0.0f;
    tokeFreq[ESkoarToke::Segno] = 0.0f;
    tokeFreq[ESkoarToke::Semicolon] = 0.0f;
    tokeFreq[ESkoarToke::SkoarpionEnd] = 0.0f;
    tokeFreq[ESkoarToke::SkoarpionSep] = 0.0f;
    tokeFreq[ESkoarToke::SkoarpionStart] = 0.0f;
    tokeFreq[ESkoarToke::Slash] = 0.0f;
    tokeFreq[ESkoarToke::String] = 0.0f;
    tokeFreq[ESkoarToke::Symbol] = 0.0f;
    tokeFreq[ESkoarToke::SymbolName] = 0.0f;
    tokeFreq[ESkoarToke::Times] = 0.0f;
    tokeFreq[ESkoarToke::True] = 0.0f;
    tokeFreq[ESkoarToke::Tuplet] = 0.0f;
    tokeFreq[ESkoarToke::Voice] = 0.0f;
    tokeFreq[ESkoarToke::Volta] = 0.0f;

    desirables = new list<ESkoarToke::Kind>[118];

    // branches
    desirables[0] = {ESkoarToke::Tuplet, ESkoarToke::QuindicesimaB, ESkoarToke::CondS, ESkoarToke::Float,
                      ESkoarToke::Choard, ESkoarToke::Caesura, ESkoarToke::Crotchets, ESkoarToke::Fine,
                      ESkoarToke::Portamento, ESkoarToke::ListS, ESkoarToke::DynForte, ESkoarToke::Crap,
                      ESkoarToke::PedalUp, ESkoarToke::Segno, ESkoarToke::False, ESkoarToke::Newline,
                      ESkoarToke::SkoarpionStart, ESkoarToke::Meter, ESkoarToke::Deref, ESkoarToke::Quarters,
                      ESkoarToke::OttavaB, ESkoarToke::HashLevel, ESkoarToke::Freq, ESkoarToke::DynSFZ,
                      ESkoarToke::DaCapo, ESkoarToke::QuindicesimaA, ESkoarToke::Bars, ESkoarToke::DalSegno,
                      ESkoarToke::Fairy, ESkoarToke::PedalDown, ESkoarToke::Comment, ESkoarToke::String,
                      ESkoarToke::Coda, ESkoarToke::Quavers, ESkoarToke::DynPiano, ESkoarToke::LoopS,
                      ESkoarToke::Symbol, ESkoarToke::Volta, ESkoarToke::LWing, ESkoarToke::OctaveShift,
                      ESkoarToke::OttavaA, ESkoarToke::Slash, ESkoarToke::DynFP, ESkoarToke::NamedNoat,
                      ESkoarToke::Carrot, ESkoarToke::True, ESkoarToke::Loco, ESkoarToke::Voice,
                      ESkoarToke::Eighths, ESkoarToke::Rep, ESkoarToke::Int};

    // skoar
    desirables[1] = {ESkoarToke::Tuplet, ESkoarToke::QuindicesimaB, ESkoarToke::CondS, ESkoarToke::Float,
                      ESkoarToke::Choard, ESkoarToke::Caesura, ESkoarToke::Crotchets, ESkoarToke::Fine,
                      ESkoarToke::Portamento, ESkoarToke::ListS, ESkoarToke::DynForte, ESkoarToke::Crap,
                      ESkoarToke::PedalUp, ESkoarToke::Segno, ESkoarToke::False, ESkoarToke::Eof,
                      ESkoarToke::Newline, ESkoarToke::SkoarpionStart, ESkoarToke::Meter, ESkoarToke::Deref,
                      ESkoarToke::Quarters, ESkoarToke::OttavaB, ESkoarToke::HashLevel, ESkoarToke::Freq,
                      ESkoarToke::DynSFZ, ESkoarToke::DaCapo, ESkoarToke::QuindicesimaA, ESkoarToke::Bars,
                      ESkoarToke::DalSegno, ESkoarToke::Fairy, ESkoarToke::PedalDown, ESkoarToke::Comment,
                      ESkoarToke::String, ESkoarToke::Coda, ESkoarToke::Quavers, ESkoarToke::DynPiano,
                      ESkoarToke::LoopS, ESkoarToke::Volta, ESkoarToke::Symbol, ESkoarToke::LWing,
                      ESkoarToke::OctaveShift, ESkoarToke::OttavaA, ESkoarToke::Slash, ESkoarToke::DynFP,
                      ESkoarToke::NamedNoat, ESkoarToke::Carrot, ESkoarToke::True, ESkoarToke::Loco,
                      ESkoarToke::Voice, ESkoarToke::Eighths, ESkoarToke::Rep, ESkoarToke::Int};

    // branch
    desirables[2] = {ESkoarToke::Tuplet, ESkoarToke::QuindicesimaB, ESkoarToke::CondS, ESkoarToke::Float,
                      ESkoarToke::Choard, ESkoarToke::Caesura, ESkoarToke::Crotchets, ESkoarToke::Fine,
                      ESkoarToke::Portamento, ESkoarToke::ListS, ESkoarToke::DynForte, ESkoarToke::Crap,
                      ESkoarToke::PedalUp, ESkoarToke::Segno, ESkoarToke::False, ESkoarToke::Newline,
                      ESkoarToke::SkoarpionStart, ESkoarToke::Meter, ESkoarToke::Deref, ESkoarToke::Quarters,
                      ESkoarToke::OttavaB, ESkoarToke::HashLevel, ESkoarToke::Freq, ESkoarToke::DynSFZ,
                      ESkoarToke::DaCapo, ESkoarToke::QuindicesimaA, ESkoarToke::Bars, ESkoarToke::DalSegno,
                      ESkoarToke::Fairy, ESkoarToke::PedalDown, ESkoarToke::Comment, ESkoarToke::String,
                      ESkoarToke::Coda, ESkoarToke::Quavers, ESkoarToke::DynPiano, ESkoarToke::LoopS,
                      ESkoarToke::Symbol, ESkoarToke::Volta, ESkoarToke::LWing, ESkoarToke::OctaveShift,
                      ESkoarToke::OttavaA, ESkoarToke::Slash, ESkoarToke::DynFP, ESkoarToke::NamedNoat,
                      ESkoarToke::Carrot, ESkoarToke::True, ESkoarToke::Loco, ESkoarToke::Voice,
                      ESkoarToke::Eighths, ESkoarToke::Rep, ESkoarToke::Int};

    // optional_voice
    desirables[3] = {ESkoarToke::Voice};

    // phrases
    desirables[4] = {ESkoarToke::Tuplet, ESkoarToke::QuindicesimaB, ESkoarToke::CondS, ESkoarToke::Float,
                      ESkoarToke::Choard, ESkoarToke::Caesura, ESkoarToke::Crotchets, ESkoarToke::Fine,
                      ESkoarToke::Portamento, ESkoarToke::ListS, ESkoarToke::DynForte, ESkoarToke::Crap,
                      ESkoarToke::PedalUp, ESkoarToke::Segno, ESkoarToke::False, ESkoarToke::SkoarpionStart,
                      ESkoarToke::Meter, ESkoarToke::Deref, ESkoarToke::Quarters, ESkoarToke::OttavaB,
                      ESkoarToke::HashLevel, ESkoarToke::Freq, ESkoarToke::DynSFZ, ESkoarToke::DaCapo,
                      ESkoarToke::QuindicesimaA, ESkoarToke::Bars, ESkoarToke::DalSegno, ESkoarToke::Fairy,
                      ESkoarToke::PedalDown, ESkoarToke::Comment, ESkoarToke::String, ESkoarToke::Coda,
                      ESkoarToke::Quavers, ESkoarToke::DynPiano, ESkoarToke::Symbol, ESkoarToke::Volta,
                      ESkoarToke::LoopS, ESkoarToke::LWing, ESkoarToke::OctaveShift, ESkoarToke::OttavaA,
                      ESkoarToke::Slash, ESkoarToke::DynFP, ESkoarToke::NamedNoat, ESkoarToke::Carrot,
                      ESkoarToke::True, ESkoarToke::Loco, ESkoarToke::Eighths, ESkoarToke::Rep,
                      ESkoarToke::Int};

    // phrasey
    desirables[5] = {ESkoarToke::Comment};
    desirables[6] = {ESkoarToke::Fine, ESkoarToke::Bars, ESkoarToke::Coda, ESkoarToke::Segno,
                      ESkoarToke::Volta};
    desirables[7] = {ESkoarToke::Meter};
    desirables[8] = {ESkoarToke::Tuplet, ESkoarToke::QuindicesimaB, ESkoarToke::CondS, ESkoarToke::Float,
                      ESkoarToke::Choard, ESkoarToke::Caesura, ESkoarToke::Portamento, ESkoarToke::ListS,
                      ESkoarToke::DynForte, ESkoarToke::Crap, ESkoarToke::PedalUp, ESkoarToke::False,
                      ESkoarToke::SkoarpionStart, ESkoarToke::Deref, ESkoarToke::OttavaB, ESkoarToke::HashLevel,
                      ESkoarToke::Freq, ESkoarToke::DynSFZ, ESkoarToke::QuindicesimaA, ESkoarToke::Fairy,
                      ESkoarToke::PedalDown, ESkoarToke::String, ESkoarToke::DynPiano, ESkoarToke::Symbol,
                      ESkoarToke::LoopS, ESkoarToke::LWing, ESkoarToke::OctaveShift, ESkoarToke::OttavaA,
                      ESkoarToke::DynFP, ESkoarToke::NamedNoat, ESkoarToke::Carrot, ESkoarToke::True,
                      ESkoarToke::Loco, ESkoarToke::Rep, ESkoarToke::Int};
    desirables[9] = {ESkoarToke::DalSegno, ESkoarToke::DaCapo};
    desirables[10] = {ESkoarToke::Crotchets, ESkoarToke::Quarters, ESkoarToke::Eighths, ESkoarToke::Quavers,
                      ESkoarToke::Slash};

    // marker
    desirables[11] = {ESkoarToke::Segno};
    desirables[12] = {ESkoarToke::Fine};
    desirables[13] = {ESkoarToke::Coda};
    desirables[14] = {ESkoarToke::Volta};
    desirables[15] = {ESkoarToke::Bars};

    // expr
    desirables[16] = {ESkoarToke::Tuplet, ESkoarToke::QuindicesimaB, ESkoarToke::CondS, ESkoarToke::Float,
                      ESkoarToke::Choard, ESkoarToke::Caesura, ESkoarToke::Portamento, ESkoarToke::ListS,
                      ESkoarToke::DynForte, ESkoarToke::Crap, ESkoarToke::PedalUp, ESkoarToke::False,
                      ESkoarToke::SkoarpionStart, ESkoarToke::Deref, ESkoarToke::OttavaB, ESkoarToke::HashLevel,
                      ESkoarToke::Freq, ESkoarToke::DynSFZ, ESkoarToke::QuindicesimaA, ESkoarToke::Fairy,
                      ESkoarToke::PedalDown, ESkoarToke::String, ESkoarToke::DynPiano, ESkoarToke::Symbol,
                      ESkoarToke::LoopS, ESkoarToke::LWing, ESkoarToke::OctaveShift, ESkoarToke::OttavaA,
                      ESkoarToke::DynFP, ESkoarToke::NamedNoat, ESkoarToke::Carrot, ESkoarToke::True,
                      ESkoarToke::Loco, ESkoarToke::Rep, ESkoarToke::Int};

    // dal_goto
    desirables[17] = {ESkoarToke::DaCapo};
    desirables[18] = {ESkoarToke::DalSegno};

    // beat
    desirables[19] = {ESkoarToke::Crotchets};
    desirables[20] = {ESkoarToke::Quavers};
    desirables[21] = {ESkoarToke::Quarters};
    desirables[22] = {ESkoarToke::Eighths};
    desirables[23] = {ESkoarToke::Slash};

    // skrp_sig
    desirables[24] = {ESkoarToke::ArgSpec};
    desirables[25] = {ESkoarToke::SymbolName};

    // skrp_suffix
    desirables[26] = {ESkoarToke::Tuplet, ESkoarToke::QuindicesimaB, ESkoarToke::CondS, ESkoarToke::Float,
                      ESkoarToke::Choard, ESkoarToke::Caesura, ESkoarToke::Crotchets, ESkoarToke::Fine,
                      ESkoarToke::Portamento, ESkoarToke::ListS, ESkoarToke::DynForte, ESkoarToke::Crap,
                      ESkoarToke::PedalUp, ESkoarToke::Segno, ESkoarToke::False, ESkoarToke::Newline,
                      ESkoarToke::SkoarpionStart, ESkoarToke::Meter, ESkoarToke::Deref, ESkoarToke::Quarters,
                      ESkoarToke::OttavaB, ESkoarToke::HashLevel, ESkoarToke::Freq, ESkoarToke::DynSFZ,
                      ESkoarToke::DaCapo, ESkoarToke::QuindicesimaA, ESkoarToke::Bars, ESkoarToke::DalSegno,
                      ESkoarToke::Fairy, ESkoarToke::PedalDown, ESkoarToke::Comment, ESkoarToke::String,
                      ESkoarToke::Coda, ESkoarToke::Quavers, ESkoarToke::DynPiano, ESkoarToke::LoopS,
                      ESkoarToke::Symbol, ESkoarToke::Volta, ESkoarToke::LWing, ESkoarToke::OctaveShift,
                      ESkoarToke::OttavaA, ESkoarToke::Slash, ESkoarToke::DynFP, ESkoarToke::NamedNoat,
                      ESkoarToke::Carrot, ESkoarToke::True, ESkoarToke::Loco, ESkoarToke::Voice,
                      ESkoarToke::SkoarpionEnd, ESkoarToke::Eighths, ESkoarToke::Rep, ESkoarToke::Int};

    // skoarpion
    desirables[27] = {ESkoarToke::SkoarpionStart};

    // opt_arg_spec
    desirables[28] = {ESkoarToke::ArgSpec};

    // skrp_lines
    desirables[29] = {ESkoarToke::Tuplet, ESkoarToke::QuindicesimaB, ESkoarToke::CondS, ESkoarToke::Float,
                      ESkoarToke::Choard, ESkoarToke::Caesura, ESkoarToke::Crotchets, ESkoarToke::Fine,
                      ESkoarToke::Portamento, ESkoarToke::ListS, ESkoarToke::DynForte, ESkoarToke::Crap,
                      ESkoarToke::PedalUp, ESkoarToke::Segno, ESkoarToke::False, ESkoarToke::Newline,
                      ESkoarToke::SkoarpionStart, ESkoarToke::Meter, ESkoarToke::Deref, ESkoarToke::Quarters,
                      ESkoarToke::OttavaB, ESkoarToke::HashLevel, ESkoarToke::Freq, ESkoarToke::DynSFZ,
                      ESkoarToke::DaCapo, ESkoarToke::QuindicesimaA, ESkoarToke::Bars, ESkoarToke::DalSegno,
                      ESkoarToke::Fairy, ESkoarToke::PedalDown, ESkoarToke::Comment, ESkoarToke::String,
                      ESkoarToke::Coda, ESkoarToke::Quavers, ESkoarToke::DynPiano, ESkoarToke::LoopS,
                      ESkoarToke::Symbol, ESkoarToke::Volta, ESkoarToke::LWing, ESkoarToke::OctaveShift,
                      ESkoarToke::OttavaA, ESkoarToke::Slash, ESkoarToke::DynFP, ESkoarToke::NamedNoat,
                      ESkoarToke::Carrot, ESkoarToke::True, ESkoarToke::Loco, ESkoarToke::Voice,
                      ESkoarToke::SkoarpionEnd, ESkoarToke::Eighths, ESkoarToke::Rep, ESkoarToke::Int};

    // skrp_moar_lines
    desirables[30] = {ESkoarToke::Newline};

    // listy_suffix
    desirables[31] = {ESkoarToke::Tuplet, ESkoarToke::QuindicesimaB, ESkoarToke::CondS, ESkoarToke::Float,
                      ESkoarToke::Choard, ESkoarToke::Caesura, ESkoarToke::Portamento, ESkoarToke::ListS,
                      ESkoarToke::DynForte, ESkoarToke::Crap, ESkoarToke::PedalUp, ESkoarToke::False,
                      ESkoarToke::SkoarpionStart, ESkoarToke::Deref, ESkoarToke::OttavaB, ESkoarToke::HashLevel,
                      ESkoarToke::Freq, ESkoarToke::DynSFZ, ESkoarToke::QuindicesimaA, ESkoarToke::Fairy,
                      ESkoarToke::PedalDown, ESkoarToke::String, ESkoarToke::DynPiano, ESkoarToke::Symbol,
                      ESkoarToke::LoopS, ESkoarToke::LWing, ESkoarToke::OctaveShift, ESkoarToke::OttavaA,
                      ESkoarToke::DynFP, ESkoarToke::NamedNoat, ESkoarToke::Carrot, ESkoarToke::True,
                      ESkoarToke::Loco, ESkoarToke::Rep, ESkoarToke::Int};
    desirables[32] = {ESkoarToke::ListE};

    // listy
    desirables[33] = {ESkoarToke::ListS};

    // listy_entries
    desirables[34] = {ESkoarToke::Tuplet, ESkoarToke::QuindicesimaB, ESkoarToke::CondS, ESkoarToke::Float,
                      ESkoarToke::Choard, ESkoarToke::Caesura, ESkoarToke::Portamento, ESkoarToke::ListS,
                      ESkoarToke::DynForte, ESkoarToke::Crap, ESkoarToke::PedalUp, ESkoarToke::False,
                      ESkoarToke::SkoarpionStart, ESkoarToke::Deref, ESkoarToke::OttavaB, ESkoarToke::HashLevel,
                      ESkoarToke::Freq, ESkoarToke::DynSFZ, ESkoarToke::QuindicesimaA, ESkoarToke::Fairy,
                      ESkoarToke::PedalDown, ESkoarToke::String, ESkoarToke::DynPiano, ESkoarToke::Symbol,
                      ESkoarToke::LoopS, ESkoarToke::LWing, ESkoarToke::OctaveShift, ESkoarToke::OttavaA,
                      ESkoarToke::DynFP, ESkoarToke::NamedNoat, ESkoarToke::Carrot, ESkoarToke::True,
                      ESkoarToke::Loco, ESkoarToke::Rep, ESkoarToke::Int};

    // moar_listy_entries
    desirables[35] = {ESkoarToke::ListSep};
    desirables[36] = {ESkoarToke::Newline};

    // coda
    desirables[37] = {ESkoarToke::Coda};

    // optional_al_coda
    desirables[38] = {ESkoarToke::AlCoda};

    // al_x
    desirables[39] = {ESkoarToke::AlCoda};
    desirables[40] = {ESkoarToke::AlSegno};
    desirables[41] = {ESkoarToke::AlFine};

    // dynamic
    desirables[42] = {ESkoarToke::DynPiano};
    desirables[43] = {ESkoarToke::DynForte};
    desirables[44] = {ESkoarToke::DynSFZ};
    desirables[45] = {ESkoarToke::DynFP};

    // musical_keyword
    desirables[46] = {ESkoarToke::DynPiano, ESkoarToke::DynForte, ESkoarToke::DynSFZ, ESkoarToke::DynFP};
    desirables[47] = {ESkoarToke::QuindicesimaA, ESkoarToke::Loco, ESkoarToke::OttavaB, ESkoarToke::QuindicesimaB,
                      ESkoarToke::OctaveShift, ESkoarToke::OttavaA};
    desirables[48] = {ESkoarToke::PedalUp, ESkoarToke::PedalDown};
    desirables[49] = {ESkoarToke::Carrot, ESkoarToke::Rep, ESkoarToke::Portamento};

    // ottavas
    desirables[50] = {ESkoarToke::OctaveShift};
    desirables[51] = {ESkoarToke::OttavaA};
    desirables[52] = {ESkoarToke::OttavaB};
    desirables[53] = {ESkoarToke::QuindicesimaA};
    desirables[54] = {ESkoarToke::QuindicesimaB};
    desirables[55] = {ESkoarToke::Loco};

    // pedally
    desirables[56] = {ESkoarToke::PedalDown};
    desirables[57] = {ESkoarToke::PedalUp};

    // musical_keyword_misc
    desirables[58] = {ESkoarToke::Rep};
    desirables[59] = {ESkoarToke::Portamento};
    desirables[60] = {ESkoarToke::Carrot};

    // cthulhu
    desirables[61] = {ESkoarToke::LWing};

    // nouny
    desirables[62] = {ESkoarToke::LWing};
    desirables[63] = {ESkoarToke::CondS};
    desirables[64] = {ESkoarToke::LoopS};
    desirables[65] = {ESkoarToke::Tuplet, ESkoarToke::Fairy, ESkoarToke::Float, ESkoarToke::String,
                      ESkoarToke::Choard, ESkoarToke::Caesura, ESkoarToke::NamedNoat, ESkoarToke::True,
                      ESkoarToke::HashLevel, ESkoarToke::Freq, ESkoarToke::Crap, ESkoarToke::Int,
                      ESkoarToke::Symbol, ESkoarToke::False};
    desirables[66] = {ESkoarToke::QuindicesimaA, ESkoarToke::OctaveShift, ESkoarToke::QuindicesimaB, ESkoarToke::PedalDown,
                      ESkoarToke::OttavaA, ESkoarToke::DynFP, ESkoarToke::Carrot, ESkoarToke::Loco,
                      ESkoarToke::OttavaB, ESkoarToke::Portamento, ESkoarToke::DynForte, ESkoarToke::DynSFZ,
                      ESkoarToke::Rep, ESkoarToke::PedalUp, ESkoarToke::DynPiano};
    desirables[67] = {ESkoarToke::ListS};
    desirables[68] = {ESkoarToke::Deref};
    desirables[69] = {ESkoarToke::SkoarpionStart};

    // conditional
    desirables[70] = {ESkoarToke::CondS};

    // loop
    desirables[71] = {ESkoarToke::LoopS};

    // nouny_literal
    desirables[72] = {ESkoarToke::Tuplet};
    desirables[73] = {ESkoarToke::Caesura};
    desirables[74] = {ESkoarToke::Freq};
    desirables[75] = {ESkoarToke::Int};
    desirables[76] = {ESkoarToke::Float};
    desirables[77] = {ESkoarToke::String};
    desirables[78] = {ESkoarToke::Choard};
    desirables[79] = {ESkoarToke::NamedNoat};
    desirables[80] = {ESkoarToke::Symbol};
    desirables[81] = {ESkoarToke::Fairy};
    desirables[82] = {ESkoarToke::HashLevel};
    desirables[83] = {ESkoarToke::False};
    desirables[84] = {ESkoarToke::True};
    desirables[85] = {ESkoarToke::Crap};

    // deref
    desirables[86] = {ESkoarToke::Deref};

    // deref_prime
    desirables[87] = {ESkoarToke::MsgNameWithArgs};
    desirables[88] = {ESkoarToke::MsgName};

    // msgable
    desirables[89] = {ESkoarToke::Tuplet, ESkoarToke::QuindicesimaB, ESkoarToke::CondS, ESkoarToke::Float,
                      ESkoarToke::Choard, ESkoarToke::Caesura, ESkoarToke::Portamento, ESkoarToke::ListS,
                      ESkoarToke::DynForte, ESkoarToke::Crap, ESkoarToke::PedalUp, ESkoarToke::False,
                      ESkoarToke::SkoarpionStart, ESkoarToke::Deref, ESkoarToke::OttavaB, ESkoarToke::HashLevel,
                      ESkoarToke::Freq, ESkoarToke::DynSFZ, ESkoarToke::QuindicesimaA, ESkoarToke::Fairy,
                      ESkoarToke::PedalDown, ESkoarToke::String, ESkoarToke::Symbol, ESkoarToke::DynPiano,
                      ESkoarToke::LoopS, ESkoarToke::LWing, ESkoarToke::OctaveShift, ESkoarToke::OttavaA,
                      ESkoarToke::DynFP, ESkoarToke::NamedNoat, ESkoarToke::Carrot, ESkoarToke::True,
                      ESkoarToke::Loco, ESkoarToke::Rep, ESkoarToke::Int};

    // expr_prime
    desirables[90] = {ESkoarToke::AssOp};
    desirables[91] = {ESkoarToke::MathOp};
    desirables[92] = {ESkoarToke::BooleanOp};
    desirables[93] = {ESkoarToke::Times};

    // assignment
    desirables[94] = {ESkoarToke::AssOp};

    // math
    desirables[95] = {ESkoarToke::MathOp};

    // boolean
    desirables[96] = {ESkoarToke::BooleanOp};

    // times
    desirables[97] = {ESkoarToke::Times};

    // settable
    desirables[98] = {ESkoarToke::Caesura};
    desirables[99] = {ESkoarToke::Symbol};
    desirables[100] = {ESkoarToke::ListS};
    desirables[101] = {ESkoarToke::Quarters};
    desirables[102] = {ESkoarToke::Eighths};
    desirables[103] = {ESkoarToke::Fairy};

    // msg_chain_node
    desirables[104] = {ESkoarToke::MsgOp};

    // msg
    desirables[105] = {ESkoarToke::MsgNameWithArgs};
    desirables[106] = {ESkoarToke::MsgName};
    desirables[107] = {ESkoarToke::ListS};
    desirables[108] = {ESkoarToke::LoopS};

    // cthulhu_prime
    desirables[109] = {ESkoarToke::Tuplet, ESkoarToke::QuindicesimaB, ESkoarToke::CondS, ESkoarToke::Float,
                      ESkoarToke::Choard, ESkoarToke::Caesura, ESkoarToke::Portamento, ESkoarToke::ListS,
                      ESkoarToke::DynForte, ESkoarToke::Crap, ESkoarToke::PedalUp, ESkoarToke::False,
                      ESkoarToke::SkoarpionStart, ESkoarToke::Deref, ESkoarToke::OttavaB, ESkoarToke::HashLevel,
                      ESkoarToke::Freq, ESkoarToke::DynSFZ, ESkoarToke::QuindicesimaA, ESkoarToke::Fairy,
                      ESkoarToke::PedalDown, ESkoarToke::String, ESkoarToke::DynPiano, ESkoarToke::Symbol,
                      ESkoarToke::LoopS, ESkoarToke::LWing, ESkoarToke::OctaveShift, ESkoarToke::OttavaA,
                      ESkoarToke::DynFP, ESkoarToke::NamedNoat, ESkoarToke::Carrot, ESkoarToke::True,
                      ESkoarToke::Loco, ESkoarToke::Rep, ESkoarToke::Int};
    desirables[110] = {ESkoarToke::Nosey};

    // cond_ifs
    desirables[111] = {ESkoarToke::Tuplet, ESkoarToke::QuindicesimaB, ESkoarToke::CondS, ESkoarToke::Float,
                      ESkoarToke::Choard, ESkoarToke::Caesura, ESkoarToke::Portamento, ESkoarToke::ListS,
                      ESkoarToke::DynForte, ESkoarToke::Crap, ESkoarToke::PedalUp, ESkoarToke::False,
                      ESkoarToke::SkoarpionStart, ESkoarToke::Deref, ESkoarToke::OttavaB, ESkoarToke::HashLevel,
                      ESkoarToke::Freq, ESkoarToke::DynSFZ, ESkoarToke::QuindicesimaA, ESkoarToke::Fairy,
                      ESkoarToke::PedalDown, ESkoarToke::String, ESkoarToke::DynPiano, ESkoarToke::Symbol,
                      ESkoarToke::LoopS, ESkoarToke::LWing, ESkoarToke::OctaveShift, ESkoarToke::OttavaA,
                      ESkoarToke::DynFP, ESkoarToke::NamedNoat, ESkoarToke::Carrot, ESkoarToke::True,
                      ESkoarToke::Loco, ESkoarToke::Voice, ESkoarToke::Rep, ESkoarToke::Int};

    // cond_if
    desirables[112] = {ESkoarToke::Tuplet, ESkoarToke::QuindicesimaB, ESkoarToke::CondS, ESkoarToke::Float,
                      ESkoarToke::Choard, ESkoarToke::Caesura, ESkoarToke::Portamento, ESkoarToke::ListS,
                      ESkoarToke::DynForte, ESkoarToke::Crap, ESkoarToke::PedalUp, ESkoarToke::False,
                      ESkoarToke::SkoarpionStart, ESkoarToke::Deref, ESkoarToke::OttavaB, ESkoarToke::HashLevel,
                      ESkoarToke::Freq, ESkoarToke::DynSFZ, ESkoarToke::QuindicesimaA, ESkoarToke::Fairy,
                      ESkoarToke::PedalDown, ESkoarToke::String, ESkoarToke::DynPiano, ESkoarToke::Symbol,
                      ESkoarToke::LoopS, ESkoarToke::LWing, ESkoarToke::OctaveShift, ESkoarToke::OttavaA,
                      ESkoarToke::DynFP, ESkoarToke::NamedNoat, ESkoarToke::Carrot, ESkoarToke::True,
                      ESkoarToke::Loco, ESkoarToke::Voice, ESkoarToke::Rep, ESkoarToke::Int};

    // cond_ifs_suffix
    desirables[113] = {ESkoarToke::Newline};

    // if_body
    desirables[114] = {ESkoarToke::Tuplet, ESkoarToke::QuindicesimaB, ESkoarToke::CondS, ESkoarToke::Float,
                      ESkoarToke::Choard, ESkoarToke::Caesura, ESkoarToke::Crotchets, ESkoarToke::Fine,
                      ESkoarToke::Portamento, ESkoarToke::ListS, ESkoarToke::DynForte, ESkoarToke::Crap,
                      ESkoarToke::PedalUp, ESkoarToke::Segno, ESkoarToke::False, ESkoarToke::CondE,
                      ESkoarToke::Newline, ESkoarToke::SkoarpionStart, ESkoarToke::Meter, ESkoarToke::Deref,
                      ESkoarToke::Quarters, ESkoarToke::OttavaB, ESkoarToke::HashLevel, ESkoarToke::Freq,
                      ESkoarToke::DynSFZ, ESkoarToke::DaCapo, ESkoarToke::QuindicesimaA, ESkoarToke::Bars,
                      ESkoarToke::DalSegno, ESkoarToke::Fairy, ESkoarToke::PedalDown, ESkoarToke::Comment,
                      ESkoarToke::String, ESkoarToke::Coda, ESkoarToke::Quavers, ESkoarToke::DynPiano,
                      ESkoarToke::LoopS, ESkoarToke::Symbol, ESkoarToke::Volta, ESkoarToke::LWing,
                      ESkoarToke::CondIf, ESkoarToke::OctaveShift, ESkoarToke::OttavaA, ESkoarToke::Slash,
                      ESkoarToke::DynFP, ESkoarToke::NamedNoat, ESkoarToke::Carrot, ESkoarToke::True,
                      ESkoarToke::Loco, ESkoarToke::Eighths, ESkoarToke::Rep, ESkoarToke::Int};

    // cond_else
    desirables[115] = {ESkoarToke::CondIf};

    // loop_body
    desirables[116] = {ESkoarToke::Tuplet, ESkoarToke::QuindicesimaB, ESkoarToke::CondS, ESkoarToke::Float,
                      ESkoarToke::Choard, ESkoarToke::Caesura, ESkoarToke::Crotchets, ESkoarToke::Fine,
                      ESkoarToke::Portamento, ESkoarToke::ListS, ESkoarToke::DynForte, ESkoarToke::Crap,
                      ESkoarToke::LoopSep, ESkoarToke::PedalUp, ESkoarToke::Segno, ESkoarToke::False,
                      ESkoarToke::SkoarpionStart, ESkoarToke::Meter, ESkoarToke::Deref, ESkoarToke::Quarters,
                      ESkoarToke::OttavaB, ESkoarToke::HashLevel, ESkoarToke::Freq, ESkoarToke::DynSFZ,
                      ESkoarToke::DaCapo, ESkoarToke::QuindicesimaA, ESkoarToke::Bars, ESkoarToke::DalSegno,
                      ESkoarToke::Fairy, ESkoarToke::PedalDown, ESkoarToke::Comment, ESkoarToke::String,
                      ESkoarToke::Coda, ESkoarToke::Quavers, ESkoarToke::DynPiano, ESkoarToke::LoopS,
                      ESkoarToke::Symbol, ESkoarToke::Volta, ESkoarToke::LWing, ESkoarToke::LoopE,
                      ESkoarToke::OctaveShift, ESkoarToke::OttavaA, ESkoarToke::Slash, ESkoarToke::DynFP,
                      ESkoarToke::NamedNoat, ESkoarToke::Carrot, ESkoarToke::True, ESkoarToke::Loco,
                      ESkoarToke::Eighths, ESkoarToke::Rep, ESkoarToke::Int};

    // loop_condition
    desirables[117] = {ESkoarToke::LoopSep};
}


static SkoarStats skoarStats;

bool localCmp(const ESkoarToke::Kind &a, const ESkoarToke::Kind &b) {
    return skoarStats.tokeFreq[a] > skoarStats.tokeFreq[b]; 
}
    
void localSortDesirables() {
    for (int i = 0; i < 118; ++i) {
        skoarStats.desirables[i].sort(localCmp);    
    }
}

SkoarNoad* SkoarParser::branches(SkoarNoad* parent) {
    SkoarNoad* noad = parent;
    list<ESkoarToke::Kind>* desires = nullptr;

    if (++deep > 1000) {
        this->fail_too_deep();
    }

    // branches -> branch branches
    desires = &skoarStats.desirables[0];
    if (toker->sees(desires) != nullptr) {
        noad->add_noad(this->branch(noad));
        this->branches(noad);
        --deep;
        return noad;
    }

    // <e>
    --deep;
    return noad;
}

SkoarNoad* SkoarParser::skoar(SkoarNoad* parent) {
    SkoarNoad* noad = SkoarNoad::New<ESkoarNoad::skoar>(wstring(L"skoar"), parent);;
    list<ESkoarToke::Kind>* desires = nullptr;

    if (++deep > 1000) {
        this->fail_too_deep();
    }

    // skoar -> branches
    desires = &skoarStats.desirables[1];
    if (toker->sees(desires) != nullptr) {
        this->branches(noad);
        --deep;
        return noad;
    }

    // Error State
    this->fail();
    return nullptr;
}

SkoarNoad* SkoarParser::branch(SkoarNoad* parent) {
    SkoarNoad* noad = SkoarNoad::New<ESkoarNoad::branch>(wstring(L"branch"), parent);;
    list<ESkoarToke::Kind>* desires = nullptr;

    if (++deep > 1000) {
        this->fail_too_deep();
    }

    // branch -> optional_voice phrases Newline
    desires = &skoarStats.desirables[2];
    if (toker->sees(desires) != nullptr) {
        this->optional_voice(noad);
        this->phrases(noad);
        skoarStats.tokeFreq[ESkoarToke::Newline] += 0.1f;
        noad->add_toke(L"Toke_Newline", toker->burn(ESkoarToke::Newline));
        --deep;
        return noad;
    }

    // Error State
    this->fail();
    return nullptr;
}

SkoarNoad* SkoarParser::optional_voice(SkoarNoad* parent) {
    SkoarNoad* noad = parent;
    list<ESkoarToke::Kind>* desires = nullptr;

    if (++deep > 1000) {
        this->fail_too_deep();
    }

    // optional_voice -> Voice
    desires = &skoarStats.desirables[3];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::Voice] += 0.1f;
        noad->add_toke(L"Toke_Voice", toker->burn(ESkoarToke::Voice));
        --deep;
        return noad;
    }

    // <e>
    --deep;
    return noad;
}

SkoarNoad* SkoarParser::phrases(SkoarNoad* parent) {
    SkoarNoad* noad = parent;
    list<ESkoarToke::Kind>* desires = nullptr;

    if (++deep > 1000) {
        this->fail_too_deep();
    }

    // phrases -> phrasey phrases
    desires = &skoarStats.desirables[4];
    if (toker->sees(desires) != nullptr) {
        this->phrasey(noad);
        this->phrases(noad);
        --deep;
        return noad;
    }

    // <e>
    --deep;
    return noad;
}

SkoarNoad* SkoarParser::phrasey(SkoarNoad* parent) {
    SkoarNoad* noad = parent;
    list<ESkoarToke::Kind>* desires = nullptr;

    if (++deep > 1000) {
        this->fail_too_deep();
    }

    // phrasey -> Comment
    desires = &skoarStats.desirables[5];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::Comment] += 0.1f;
        noad->add_toke(L"Toke_Comment", toker->burn(ESkoarToke::Comment));
        --deep;
        return noad;
    }

    // phrasey -> marker
    desires = &skoarStats.desirables[6];
    if (toker->sees(desires) != nullptr) {
        noad->add_noad(this->marker(noad));
        --deep;
        return noad;
    }

    // phrasey -> Meter
    desires = &skoarStats.desirables[7];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::Meter] += 0.1f;
        noad->add_toke(L"Toke_Meter", toker->burn(ESkoarToke::Meter));
        --deep;
        return noad;
    }

    // phrasey -> expr
    desires = &skoarStats.desirables[8];
    if (toker->sees(desires) != nullptr) {
        noad->add_noad(this->expr(noad));
        --deep;
        return noad;
    }

    // phrasey -> dal_goto
    desires = &skoarStats.desirables[9];
    if (toker->sees(desires) != nullptr) {
        noad->add_noad(this->dal_goto(noad));
        --deep;
        return noad;
    }

    // phrasey -> beat
    desires = &skoarStats.desirables[10];
    if (toker->sees(desires) != nullptr) {
        noad->add_noad(this->beat(noad));
        --deep;
        return noad;
    }

    // Error State
    this->fail();
    return nullptr;
}

SkoarNoad* SkoarParser::marker(SkoarNoad* parent) {
    SkoarNoad* noad = SkoarNoad::New<ESkoarNoad::marker>(wstring(L"marker"), parent);;
    list<ESkoarToke::Kind>* desires = nullptr;

    if (++deep > 1000) {
        this->fail_too_deep();
    }

    // marker -> Segno
    desires = &skoarStats.desirables[11];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::Segno] += 0.1f;
        noad->add_toke(L"Toke_Segno", toker->burn(ESkoarToke::Segno));
        --deep;
        return noad;
    }

    // marker -> Fine
    desires = &skoarStats.desirables[12];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::Fine] += 0.1f;
        noad->add_toke(L"Toke_Fine", toker->burn(ESkoarToke::Fine));
        --deep;
        return noad;
    }

    // marker -> coda
    desires = &skoarStats.desirables[13];
    if (toker->sees(desires) != nullptr) {
        noad->add_noad(this->coda(noad));
        --deep;
        return noad;
    }

    // marker -> Volta
    desires = &skoarStats.desirables[14];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::Volta] += 0.1f;
        noad->add_toke(L"Toke_Volta", toker->burn(ESkoarToke::Volta));
        --deep;
        return noad;
    }

    // marker -> Bars
    desires = &skoarStats.desirables[15];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::Bars] += 0.1f;
        noad->add_toke(L"Toke_Bars", toker->burn(ESkoarToke::Bars));
        --deep;
        return noad;
    }

    // Error State
    this->fail();
    return nullptr;
}

SkoarNoad* SkoarParser::expr(SkoarNoad* parent) {
    SkoarNoad* noad = SkoarNoad::New<ESkoarNoad::expr>(wstring(L"expr"), parent);;
    list<ESkoarToke::Kind>* desires = nullptr;

    if (++deep > 1000) {
        this->fail_too_deep();
    }

    // expr -> msgable expr_prime
    desires = &skoarStats.desirables[16];
    if (toker->sees(desires) != nullptr) {
        noad->add_noad(this->msgable(noad));
        noad->add_noad(this->expr_prime(noad));
        --deep;
        return noad;
    }

    // Error State
    this->fail();
    return nullptr;
}

SkoarNoad* SkoarParser::dal_goto(SkoarNoad* parent) {
    SkoarNoad* noad = SkoarNoad::New<ESkoarNoad::dal_goto>(wstring(L"dal_goto"), parent);;
    list<ESkoarToke::Kind>* desires = nullptr;

    if (++deep > 1000) {
        this->fail_too_deep();
    }

    // dal_goto -> DaCapo al_x
    desires = &skoarStats.desirables[17];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::DaCapo] += 0.1f;
        noad->add_toke(L"Toke_DaCapo", toker->burn(ESkoarToke::DaCapo));
        noad->add_noad(this->al_x(noad));
        --deep;
        return noad;
    }

    // dal_goto -> DalSegno al_x
    desires = &skoarStats.desirables[18];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::DalSegno] += 0.1f;
        noad->add_toke(L"Toke_DalSegno", toker->burn(ESkoarToke::DalSegno));
        noad->add_noad(this->al_x(noad));
        --deep;
        return noad;
    }

    // Error State
    this->fail();
    return nullptr;
}

SkoarNoad* SkoarParser::beat(SkoarNoad* parent) {
    SkoarNoad* noad = SkoarNoad::New<ESkoarNoad::beat>(wstring(L"beat"), parent);;
    list<ESkoarToke::Kind>* desires = nullptr;

    if (++deep > 1000) {
        this->fail_too_deep();
    }

    // beat -> Crotchets
    desires = &skoarStats.desirables[19];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::Crotchets] += 0.1f;
        noad->add_toke(L"Toke_Crotchets", toker->burn(ESkoarToke::Crotchets));
        --deep;
        return noad;
    }

    // beat -> Quavers
    desires = &skoarStats.desirables[20];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::Quavers] += 0.1f;
        noad->add_toke(L"Toke_Quavers", toker->burn(ESkoarToke::Quavers));
        --deep;
        return noad;
    }

    // beat -> Quarters
    desires = &skoarStats.desirables[21];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::Quarters] += 0.1f;
        noad->add_toke(L"Toke_Quarters", toker->burn(ESkoarToke::Quarters));
        --deep;
        return noad;
    }

    // beat -> Eighths
    desires = &skoarStats.desirables[22];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::Eighths] += 0.1f;
        noad->add_toke(L"Toke_Eighths", toker->burn(ESkoarToke::Eighths));
        --deep;
        return noad;
    }

    // beat -> Slash
    desires = &skoarStats.desirables[23];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::Slash] += 0.1f;
        noad->add_toke(L"Toke_Slash", toker->burn(ESkoarToke::Slash));
        --deep;
        return noad;
    }

    // Error State
    this->fail();
    return nullptr;
}

SkoarNoad* SkoarParser::skrp_sig(SkoarNoad* parent) {
    SkoarNoad* noad = SkoarNoad::New<ESkoarNoad::skrp_sig>(wstring(L"skrp_sig"), parent);;
    list<ESkoarToke::Kind>* desires = nullptr;

    if (++deep > 1000) {
        this->fail_too_deep();
    }

    // skrp_sig -> ArgSpec SkoarpionSep
    desires = &skoarStats.desirables[24];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::ArgSpec] += 0.1f;
        noad->add_toke(L"Toke_ArgSpec", toker->burn(ESkoarToke::ArgSpec));
        skoarStats.tokeFreq[ESkoarToke::SkoarpionSep] += 0.1f;
        noad->add_toke(L"Toke_SkoarpionSep", toker->burn(ESkoarToke::SkoarpionSep));
        --deep;
        return noad;
    }

    // skrp_sig -> SymbolName opt_arg_spec SkoarpionSep
    desires = &skoarStats.desirables[25];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::SymbolName] += 0.1f;
        noad->add_toke(L"Toke_SymbolName", toker->burn(ESkoarToke::SymbolName));
        noad->add_noad(this->opt_arg_spec(noad));
        skoarStats.tokeFreq[ESkoarToke::SkoarpionSep] += 0.1f;
        noad->add_toke(L"Toke_SkoarpionSep", toker->burn(ESkoarToke::SkoarpionSep));
        --deep;
        return noad;
    }

    // <e>
    --deep;
    return noad;
}

SkoarNoad* SkoarParser::skrp_suffix(SkoarNoad* parent) {
    SkoarNoad* noad = SkoarNoad::New<ESkoarNoad::skrp_suffix>(wstring(L"skrp_suffix"), parent);;
    list<ESkoarToke::Kind>* desires = nullptr;

    if (++deep > 1000) {
        this->fail_too_deep();
    }

    // skrp_suffix -> skrp_lines SkoarpionEnd
    desires = &skoarStats.desirables[26];
    if (toker->sees(desires) != nullptr) {
        this->skrp_lines(noad);
        skoarStats.tokeFreq[ESkoarToke::SkoarpionEnd] += 0.1f;
        noad->add_toke(L"Toke_SkoarpionEnd", toker->burn(ESkoarToke::SkoarpionEnd));
        --deep;
        return noad;
    }

    // Error State
    this->fail();
    return nullptr;
}

SkoarNoad* SkoarParser::skoarpion(SkoarNoad* parent) {
    SkoarNoad* noad = SkoarNoad::New<ESkoarNoad::skoarpion>(wstring(L"skoarpion"), parent);;
    list<ESkoarToke::Kind>* desires = nullptr;

    if (++deep > 1000) {
        this->fail_too_deep();
    }

    // skoarpion -> SkoarpionStart skrp_sig skrp_suffix
    desires = &skoarStats.desirables[27];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::SkoarpionStart] += 0.1f;
        noad->add_toke(L"Toke_SkoarpionStart", toker->burn(ESkoarToke::SkoarpionStart));
        noad->add_noad(this->skrp_sig(noad));
        noad->add_noad(this->skrp_suffix(noad));
        --deep;
        return noad;
    }

    // Error State
    this->fail();
    return nullptr;
}

SkoarNoad* SkoarParser::opt_arg_spec(SkoarNoad* parent) {
    SkoarNoad* noad = SkoarNoad::New<ESkoarNoad::opt_arg_spec>(wstring(L"opt_arg_spec"), parent);;
    list<ESkoarToke::Kind>* desires = nullptr;

    if (++deep > 1000) {
        this->fail_too_deep();
    }

    // opt_arg_spec -> ArgSpec
    desires = &skoarStats.desirables[28];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::ArgSpec] += 0.1f;
        noad->add_toke(L"Toke_ArgSpec", toker->burn(ESkoarToke::ArgSpec));
        --deep;
        return noad;
    }

    // <e>
    --deep;
    return noad;
}

SkoarNoad* SkoarParser::skrp_lines(SkoarNoad* parent) {
    SkoarNoad* noad = parent;
    list<ESkoarToke::Kind>* desires = nullptr;

    if (++deep > 1000) {
        this->fail_too_deep();
    }

    // skrp_lines -> optional_voice phrases skrp_moar_lines
    desires = &skoarStats.desirables[29];
    if (toker->sees(desires) != nullptr) {
        this->optional_voice(noad);
        this->phrases(noad);
        this->skrp_moar_lines(noad);
        --deep;
        return noad;
    }

    // Error State
    this->fail();
    return nullptr;
}

SkoarNoad* SkoarParser::skrp_moar_lines(SkoarNoad* parent) {
    SkoarNoad* noad = parent;
    list<ESkoarToke::Kind>* desires = nullptr;

    if (++deep > 1000) {
        this->fail_too_deep();
    }

    // skrp_moar_lines -> Newline skrp_lines
    desires = &skoarStats.desirables[30];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::Newline] += 0.1f;
        noad->add_toke(L"Toke_Newline", toker->burn(ESkoarToke::Newline));
        this->skrp_lines(noad);
        --deep;
        return noad;
    }

    // <e>
    --deep;
    return noad;
}

SkoarNoad* SkoarParser::listy_suffix(SkoarNoad* parent) {
    SkoarNoad* noad = parent;
    list<ESkoarToke::Kind>* desires = nullptr;

    if (++deep > 1000) {
        this->fail_too_deep();
    }

    // listy_suffix -> listy_entries ListE
    desires = &skoarStats.desirables[31];
    if (toker->sees(desires) != nullptr) {
        this->listy_entries(noad);
        skoarStats.tokeFreq[ESkoarToke::ListE] += 0.1f;
        noad->add_toke(L"Toke_ListE", toker->burn(ESkoarToke::ListE));
        --deep;
        return noad;
    }

    // listy_suffix -> ListE
    desires = &skoarStats.desirables[32];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::ListE] += 0.1f;
        noad->add_toke(L"Toke_ListE", toker->burn(ESkoarToke::ListE));
        --deep;
        return noad;
    }

    // Error State
    this->fail();
    return nullptr;
}

SkoarNoad* SkoarParser::listy(SkoarNoad* parent) {
    SkoarNoad* noad = SkoarNoad::New<ESkoarNoad::listy>(wstring(L"listy"), parent);;
    list<ESkoarToke::Kind>* desires = nullptr;

    if (++deep > 1000) {
        this->fail_too_deep();
    }

    // listy -> ListS listy_suffix
    desires = &skoarStats.desirables[33];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::ListS] += 0.1f;
        noad->add_toke(L"Toke_ListS", toker->burn(ESkoarToke::ListS));
        this->listy_suffix(noad);
        --deep;
        return noad;
    }

    // Error State
    this->fail();
    return nullptr;
}

SkoarNoad* SkoarParser::listy_entries(SkoarNoad* parent) {
    SkoarNoad* noad = parent;
    list<ESkoarToke::Kind>* desires = nullptr;

    if (++deep > 1000) {
        this->fail_too_deep();
    }

    // listy_entries -> expr moar_listy_entries
    desires = &skoarStats.desirables[34];
    if (toker->sees(desires) != nullptr) {
        noad->add_noad(this->expr(noad));
        this->moar_listy_entries(noad);
        --deep;
        return noad;
    }

    // Error State
    this->fail();
    return nullptr;
}

SkoarNoad* SkoarParser::moar_listy_entries(SkoarNoad* parent) {
    SkoarNoad* noad = parent;
    list<ESkoarToke::Kind>* desires = nullptr;

    if (++deep > 1000) {
        this->fail_too_deep();
    }

    // moar_listy_entries -> ListSep listy_entries
    desires = &skoarStats.desirables[35];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::ListSep] += 0.1f;
        noad->add_toke(L"Toke_ListSep", toker->burn(ESkoarToke::ListSep));
        this->listy_entries(noad);
        --deep;
        return noad;
    }

    // moar_listy_entries -> Newline
    desires = &skoarStats.desirables[36];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::Newline] += 0.1f;
        noad->add_toke(L"Toke_Newline", toker->burn(ESkoarToke::Newline));
        --deep;
        return noad;
    }

    // <e>
    --deep;
    return noad;
}

SkoarNoad* SkoarParser::coda(SkoarNoad* parent) {
    SkoarNoad* noad = SkoarNoad::New<ESkoarNoad::coda>(wstring(L"coda"), parent);;
    list<ESkoarToke::Kind>* desires = nullptr;

    if (++deep > 1000) {
        this->fail_too_deep();
    }

    // coda -> Coda optional_al_coda
    desires = &skoarStats.desirables[37];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::Coda] += 0.1f;
        noad->add_toke(L"Toke_Coda", toker->burn(ESkoarToke::Coda));
        noad->add_noad(this->optional_al_coda(noad));
        --deep;
        return noad;
    }

    // Error State
    this->fail();
    return nullptr;
}

SkoarNoad* SkoarParser::optional_al_coda(SkoarNoad* parent) {
    SkoarNoad* noad = SkoarNoad::New<ESkoarNoad::optional_al_coda>(wstring(L"optional_al_coda"), parent);;
    list<ESkoarToke::Kind>* desires = nullptr;

    if (++deep > 1000) {
        this->fail_too_deep();
    }

    // optional_al_coda -> AlCoda
    desires = &skoarStats.desirables[38];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::AlCoda] += 0.1f;
        noad->add_toke(L"Toke_AlCoda", toker->burn(ESkoarToke::AlCoda));
        --deep;
        return noad;
    }

    // <e>
    --deep;
    return noad;
}

SkoarNoad* SkoarParser::al_x(SkoarNoad* parent) {
    SkoarNoad* noad = SkoarNoad::New<ESkoarNoad::al_x>(wstring(L"al_x"), parent);;
    list<ESkoarToke::Kind>* desires = nullptr;

    if (++deep > 1000) {
        this->fail_too_deep();
    }

    // al_x -> AlCoda
    desires = &skoarStats.desirables[39];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::AlCoda] += 0.1f;
        noad->add_toke(L"Toke_AlCoda", toker->burn(ESkoarToke::AlCoda));
        --deep;
        return noad;
    }

    // al_x -> AlSegno
    desires = &skoarStats.desirables[40];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::AlSegno] += 0.1f;
        noad->add_toke(L"Toke_AlSegno", toker->burn(ESkoarToke::AlSegno));
        --deep;
        return noad;
    }

    // al_x -> AlFine
    desires = &skoarStats.desirables[41];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::AlFine] += 0.1f;
        noad->add_toke(L"Toke_AlFine", toker->burn(ESkoarToke::AlFine));
        --deep;
        return noad;
    }

    // <e>
    --deep;
    return noad;
}

SkoarNoad* SkoarParser::dynamic(SkoarNoad* parent) {
    SkoarNoad* noad = SkoarNoad::New<ESkoarNoad::dynamic>(wstring(L"dynamic"), parent);;
    list<ESkoarToke::Kind>* desires = nullptr;

    if (++deep > 1000) {
        this->fail_too_deep();
    }

    // dynamic -> DynPiano
    desires = &skoarStats.desirables[42];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::DynPiano] += 0.1f;
        noad->add_toke(L"Toke_DynPiano", toker->burn(ESkoarToke::DynPiano));
        --deep;
        return noad;
    }

    // dynamic -> DynForte
    desires = &skoarStats.desirables[43];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::DynForte] += 0.1f;
        noad->add_toke(L"Toke_DynForte", toker->burn(ESkoarToke::DynForte));
        --deep;
        return noad;
    }

    // dynamic -> DynSFZ
    desires = &skoarStats.desirables[44];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::DynSFZ] += 0.1f;
        noad->add_toke(L"Toke_DynSFZ", toker->burn(ESkoarToke::DynSFZ));
        --deep;
        return noad;
    }

    // dynamic -> DynFP
    desires = &skoarStats.desirables[45];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::DynFP] += 0.1f;
        noad->add_toke(L"Toke_DynFP", toker->burn(ESkoarToke::DynFP));
        --deep;
        return noad;
    }

    // Error State
    this->fail();
    return nullptr;
}

SkoarNoad* SkoarParser::musical_keyword(SkoarNoad* parent) {
    SkoarNoad* noad = SkoarNoad::New<ESkoarNoad::musical_keyword>(wstring(L"musical_keyword"), parent);;
    list<ESkoarToke::Kind>* desires = nullptr;

    if (++deep > 1000) {
        this->fail_too_deep();
    }

    // musical_keyword -> dynamic
    desires = &skoarStats.desirables[46];
    if (toker->sees(desires) != nullptr) {
        noad->add_noad(this->dynamic(noad));
        --deep;
        return noad;
    }

    // musical_keyword -> ottavas
    desires = &skoarStats.desirables[47];
    if (toker->sees(desires) != nullptr) {
        noad->add_noad(this->ottavas(noad));
        --deep;
        return noad;
    }

    // musical_keyword -> pedally
    desires = &skoarStats.desirables[48];
    if (toker->sees(desires) != nullptr) {
        noad->add_noad(this->pedally(noad));
        --deep;
        return noad;
    }

    // musical_keyword -> musical_keyword_misc
    desires = &skoarStats.desirables[49];
    if (toker->sees(desires) != nullptr) {
        noad->add_noad(this->musical_keyword_misc(noad));
        --deep;
        return noad;
    }

    // Error State
    this->fail();
    return nullptr;
}

SkoarNoad* SkoarParser::ottavas(SkoarNoad* parent) {
    SkoarNoad* noad = SkoarNoad::New<ESkoarNoad::ottavas>(wstring(L"ottavas"), parent);;
    list<ESkoarToke::Kind>* desires = nullptr;

    if (++deep > 1000) {
        this->fail_too_deep();
    }

    // ottavas -> OctaveShift
    desires = &skoarStats.desirables[50];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::OctaveShift] += 0.1f;
        noad->add_toke(L"Toke_OctaveShift", toker->burn(ESkoarToke::OctaveShift));
        --deep;
        return noad;
    }

    // ottavas -> OttavaA
    desires = &skoarStats.desirables[51];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::OttavaA] += 0.1f;
        noad->add_toke(L"Toke_OttavaA", toker->burn(ESkoarToke::OttavaA));
        --deep;
        return noad;
    }

    // ottavas -> OttavaB
    desires = &skoarStats.desirables[52];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::OttavaB] += 0.1f;
        noad->add_toke(L"Toke_OttavaB", toker->burn(ESkoarToke::OttavaB));
        --deep;
        return noad;
    }

    // ottavas -> QuindicesimaA
    desires = &skoarStats.desirables[53];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::QuindicesimaA] += 0.1f;
        noad->add_toke(L"Toke_QuindicesimaA", toker->burn(ESkoarToke::QuindicesimaA));
        --deep;
        return noad;
    }

    // ottavas -> QuindicesimaB
    desires = &skoarStats.desirables[54];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::QuindicesimaB] += 0.1f;
        noad->add_toke(L"Toke_QuindicesimaB", toker->burn(ESkoarToke::QuindicesimaB));
        --deep;
        return noad;
    }

    // ottavas -> Loco
    desires = &skoarStats.desirables[55];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::Loco] += 0.1f;
        noad->add_toke(L"Toke_Loco", toker->burn(ESkoarToke::Loco));
        --deep;
        return noad;
    }

    // Error State
    this->fail();
    return nullptr;
}

SkoarNoad* SkoarParser::pedally(SkoarNoad* parent) {
    SkoarNoad* noad = SkoarNoad::New<ESkoarNoad::pedally>(wstring(L"pedally"), parent);;
    list<ESkoarToke::Kind>* desires = nullptr;

    if (++deep > 1000) {
        this->fail_too_deep();
    }

    // pedally -> PedalDown
    desires = &skoarStats.desirables[56];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::PedalDown] += 0.1f;
        noad->add_toke(L"Toke_PedalDown", toker->burn(ESkoarToke::PedalDown));
        --deep;
        return noad;
    }

    // pedally -> PedalUp
    desires = &skoarStats.desirables[57];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::PedalUp] += 0.1f;
        noad->add_toke(L"Toke_PedalUp", toker->burn(ESkoarToke::PedalUp));
        --deep;
        return noad;
    }

    // Error State
    this->fail();
    return nullptr;
}

SkoarNoad* SkoarParser::musical_keyword_misc(SkoarNoad* parent) {
    SkoarNoad* noad = SkoarNoad::New<ESkoarNoad::musical_keyword_misc>(wstring(L"musical_keyword_misc"), parent);;
    list<ESkoarToke::Kind>* desires = nullptr;

    if (++deep > 1000) {
        this->fail_too_deep();
    }

    // musical_keyword_misc -> Rep
    desires = &skoarStats.desirables[58];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::Rep] += 0.1f;
        noad->add_toke(L"Toke_Rep", toker->burn(ESkoarToke::Rep));
        --deep;
        return noad;
    }

    // musical_keyword_misc -> Portamento
    desires = &skoarStats.desirables[59];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::Portamento] += 0.1f;
        noad->add_toke(L"Toke_Portamento", toker->burn(ESkoarToke::Portamento));
        --deep;
        return noad;
    }

    // musical_keyword_misc -> Carrot
    desires = &skoarStats.desirables[60];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::Carrot] += 0.1f;
        noad->add_toke(L"Toke_Carrot", toker->burn(ESkoarToke::Carrot));
        --deep;
        return noad;
    }

    // Error State
    this->fail();
    return nullptr;
}

SkoarNoad* SkoarParser::cthulhu(SkoarNoad* parent) {
    SkoarNoad* noad = SkoarNoad::New<ESkoarNoad::cthulhu>(wstring(L"cthulhu"), parent);;
    list<ESkoarToke::Kind>* desires = nullptr;

    if (++deep > 1000) {
        this->fail_too_deep();
    }

    // cthulhu -> LWing Semicolon cthulhu_prime
    desires = &skoarStats.desirables[61];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::LWing] += 0.1f;
        noad->add_toke(L"Toke_LWing", toker->burn(ESkoarToke::LWing));
        skoarStats.tokeFreq[ESkoarToke::Semicolon] += 0.1f;
        noad->add_toke(L"Toke_Semicolon", toker->burn(ESkoarToke::Semicolon));
        this->cthulhu_prime(noad);
        --deep;
        return noad;
    }

    // Error State
    this->fail();
    return nullptr;
}

SkoarNoad* SkoarParser::nouny(SkoarNoad* parent) {
    SkoarNoad* noad = SkoarNoad::New<ESkoarNoad::nouny>(wstring(L"nouny"), parent);;
    list<ESkoarToke::Kind>* desires = nullptr;

    if (++deep > 1000) {
        this->fail_too_deep();
    }

    // nouny -> cthulhu
    desires = &skoarStats.desirables[62];
    if (toker->sees(desires) != nullptr) {
        noad->add_noad(this->cthulhu(noad));
        --deep;
        return noad;
    }

    // nouny -> conditional
    desires = &skoarStats.desirables[63];
    if (toker->sees(desires) != nullptr) {
        noad->add_noad(this->conditional(noad));
        --deep;
        return noad;
    }

    // nouny -> loop
    desires = &skoarStats.desirables[64];
    if (toker->sees(desires) != nullptr) {
        noad->add_noad(this->loop(noad));
        --deep;
        return noad;
    }

    // nouny -> nouny_literal
    desires = &skoarStats.desirables[65];
    if (toker->sees(desires) != nullptr) {
        this->nouny_literal(noad);
        --deep;
        return noad;
    }

    // nouny -> musical_keyword
    desires = &skoarStats.desirables[66];
    if (toker->sees(desires) != nullptr) {
        noad->add_noad(this->musical_keyword(noad));
        --deep;
        return noad;
    }

    // nouny -> listy
    desires = &skoarStats.desirables[67];
    if (toker->sees(desires) != nullptr) {
        noad->add_noad(this->listy(noad));
        --deep;
        return noad;
    }

    // nouny -> deref
    desires = &skoarStats.desirables[68];
    if (toker->sees(desires) != nullptr) {
        noad->add_noad(this->deref(noad));
        --deep;
        return noad;
    }

    // nouny -> skoarpion
    desires = &skoarStats.desirables[69];
    if (toker->sees(desires) != nullptr) {
        noad->add_noad(this->skoarpion(noad));
        --deep;
        return noad;
    }

    // Error State
    this->fail();
    return nullptr;
}

SkoarNoad* SkoarParser::conditional(SkoarNoad* parent) {
    SkoarNoad* noad = SkoarNoad::New<ESkoarNoad::conditional>(wstring(L"conditional"), parent);;
    list<ESkoarToke::Kind>* desires = nullptr;

    if (++deep > 1000) {
        this->fail_too_deep();
    }

    // conditional -> CondS cond_ifs CondE
    desires = &skoarStats.desirables[70];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::CondS] += 0.1f;
        noad->add_toke(L"Toke_CondS", toker->burn(ESkoarToke::CondS));
        this->cond_ifs(noad);
        skoarStats.tokeFreq[ESkoarToke::CondE] += 0.1f;
        noad->add_toke(L"Toke_CondE", toker->burn(ESkoarToke::CondE));
        --deep;
        return noad;
    }

    // Error State
    this->fail();
    return nullptr;
}

SkoarNoad* SkoarParser::loop(SkoarNoad* parent) {
    SkoarNoad* noad = SkoarNoad::New<ESkoarNoad::loop>(wstring(L"loop"), parent);;
    list<ESkoarToke::Kind>* desires = nullptr;

    if (++deep > 1000) {
        this->fail_too_deep();
    }

    // loop -> LoopS loop_body loop_condition LoopE
    desires = &skoarStats.desirables[71];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::LoopS] += 0.1f;
        noad->add_toke(L"Toke_LoopS", toker->burn(ESkoarToke::LoopS));
        noad->add_noad(this->loop_body(noad));
        noad->add_noad(this->loop_condition(noad));
        skoarStats.tokeFreq[ESkoarToke::LoopE] += 0.1f;
        noad->add_toke(L"Toke_LoopE", toker->burn(ESkoarToke::LoopE));
        --deep;
        return noad;
    }

    // Error State
    this->fail();
    return nullptr;
}

SkoarNoad* SkoarParser::nouny_literal(SkoarNoad* parent) {
    SkoarNoad* noad = parent;
    list<ESkoarToke::Kind>* desires = nullptr;

    if (++deep > 1000) {
        this->fail_too_deep();
    }

    // nouny_literal -> Tuplet
    desires = &skoarStats.desirables[72];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::Tuplet] += 0.1f;
        noad->add_toke(L"Toke_Tuplet", toker->burn(ESkoarToke::Tuplet));
        --deep;
        return noad;
    }

    // nouny_literal -> Caesura
    desires = &skoarStats.desirables[73];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::Caesura] += 0.1f;
        noad->add_toke(L"Toke_Caesura", toker->burn(ESkoarToke::Caesura));
        --deep;
        return noad;
    }

    // nouny_literal -> Freq
    desires = &skoarStats.desirables[74];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::Freq] += 0.1f;
        noad->add_toke(L"Toke_Freq", toker->burn(ESkoarToke::Freq));
        --deep;
        return noad;
    }

    // nouny_literal -> Int
    desires = &skoarStats.desirables[75];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::Int] += 0.1f;
        noad->add_toke(L"Toke_Int", toker->burn(ESkoarToke::Int));
        --deep;
        return noad;
    }

    // nouny_literal -> Float
    desires = &skoarStats.desirables[76];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::Float] += 0.1f;
        noad->add_toke(L"Toke_Float", toker->burn(ESkoarToke::Float));
        --deep;
        return noad;
    }

    // nouny_literal -> String
    desires = &skoarStats.desirables[77];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::String] += 0.1f;
        noad->add_toke(L"Toke_String", toker->burn(ESkoarToke::String));
        --deep;
        return noad;
    }

    // nouny_literal -> Choard
    desires = &skoarStats.desirables[78];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::Choard] += 0.1f;
        noad->add_toke(L"Toke_Choard", toker->burn(ESkoarToke::Choard));
        --deep;
        return noad;
    }

    // nouny_literal -> NamedNoat
    desires = &skoarStats.desirables[79];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::NamedNoat] += 0.1f;
        noad->add_toke(L"Toke_NamedNoat", toker->burn(ESkoarToke::NamedNoat));
        --deep;
        return noad;
    }

    // nouny_literal -> Symbol
    desires = &skoarStats.desirables[80];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::Symbol] += 0.1f;
        noad->add_toke(L"Toke_Symbol", toker->burn(ESkoarToke::Symbol));
        --deep;
        return noad;
    }

    // nouny_literal -> Fairy
    desires = &skoarStats.desirables[81];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::Fairy] += 0.1f;
        noad->add_toke(L"Toke_Fairy", toker->burn(ESkoarToke::Fairy));
        --deep;
        return noad;
    }

    // nouny_literal -> HashLevel
    desires = &skoarStats.desirables[82];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::HashLevel] += 0.1f;
        noad->add_toke(L"Toke_HashLevel", toker->burn(ESkoarToke::HashLevel));
        --deep;
        return noad;
    }

    // nouny_literal -> False
    desires = &skoarStats.desirables[83];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::False] += 0.1f;
        noad->add_toke(L"Toke_False", toker->burn(ESkoarToke::False));
        --deep;
        return noad;
    }

    // nouny_literal -> True
    desires = &skoarStats.desirables[84];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::True] += 0.1f;
        noad->add_toke(L"Toke_True", toker->burn(ESkoarToke::True));
        --deep;
        return noad;
    }

    // nouny_literal -> Crap
    desires = &skoarStats.desirables[85];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::Crap] += 0.1f;
        noad->add_toke(L"Toke_Crap", toker->burn(ESkoarToke::Crap));
        --deep;
        return noad;
    }

    // Error State
    this->fail();
    return nullptr;
}

SkoarNoad* SkoarParser::deref(SkoarNoad* parent) {
    SkoarNoad* noad = SkoarNoad::New<ESkoarNoad::deref>(wstring(L"deref"), parent);;
    list<ESkoarToke::Kind>* desires = nullptr;

    if (++deep > 1000) {
        this->fail_too_deep();
    }

    // deref -> Deref deref_prime
    desires = &skoarStats.desirables[86];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::Deref] += 0.1f;
        noad->add_toke(L"Toke_Deref", toker->burn(ESkoarToke::Deref));
        this->deref_prime(noad);
        --deep;
        return noad;
    }

    // Error State
    this->fail();
    return nullptr;
}

SkoarNoad* SkoarParser::deref_prime(SkoarNoad* parent) {
    SkoarNoad* noad = parent;
    list<ESkoarToke::Kind>* desires = nullptr;

    if (++deep > 1000) {
        this->fail_too_deep();
    }

    // deref_prime -> MsgNameWithArgs listy_suffix
    desires = &skoarStats.desirables[87];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::MsgNameWithArgs] += 0.1f;
        noad->add_toke(L"Toke_MsgNameWithArgs", toker->burn(ESkoarToke::MsgNameWithArgs));
        this->listy_suffix(noad);
        --deep;
        return noad;
    }

    // deref_prime -> MsgName
    desires = &skoarStats.desirables[88];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::MsgName] += 0.1f;
        noad->add_toke(L"Toke_MsgName", toker->burn(ESkoarToke::MsgName));
        --deep;
        return noad;
    }

    // Error State
    this->fail();
    return nullptr;
}

SkoarNoad* SkoarParser::msgable(SkoarNoad* parent) {
    SkoarNoad* noad = SkoarNoad::New<ESkoarNoad::msgable>(wstring(L"msgable"), parent);;
    list<ESkoarToke::Kind>* desires = nullptr;

    if (++deep > 1000) {
        this->fail_too_deep();
    }

    // msgable -> nouny msg_chain_node
    desires = &skoarStats.desirables[89];
    if (toker->sees(desires) != nullptr) {
        noad->add_noad(this->nouny(noad));
        this->msg_chain_node(noad);
        --deep;
        return noad;
    }

    // Error State
    this->fail();
    return nullptr;
}

SkoarNoad* SkoarParser::expr_prime(SkoarNoad* parent) {
    SkoarNoad* noad = SkoarNoad::New<ESkoarNoad::expr_prime>(wstring(L"expr_prime"), parent);;
    list<ESkoarToke::Kind>* desires = nullptr;

    if (++deep > 1000) {
        this->fail_too_deep();
    }

    // expr_prime -> assignment expr_prime
    desires = &skoarStats.desirables[90];
    if (toker->sees(desires) != nullptr) {
        noad->add_noad(this->assignment(noad));
        noad->add_noad(this->expr_prime(noad));
        --deep;
        return noad;
    }

    // expr_prime -> math expr_prime
    desires = &skoarStats.desirables[91];
    if (toker->sees(desires) != nullptr) {
        noad->add_noad(this->math(noad));
        noad->add_noad(this->expr_prime(noad));
        --deep;
        return noad;
    }

    // expr_prime -> boolean
    desires = &skoarStats.desirables[92];
    if (toker->sees(desires) != nullptr) {
        noad->add_noad(this->boolean(noad));
        --deep;
        return noad;
    }

    // expr_prime -> times
    desires = &skoarStats.desirables[93];
    if (toker->sees(desires) != nullptr) {
        noad->add_noad(this->times(noad));
        --deep;
        return noad;
    }

    // <e>
    --deep;
    return noad;
}

SkoarNoad* SkoarParser::assignment(SkoarNoad* parent) {
    SkoarNoad* noad = SkoarNoad::New<ESkoarNoad::assignment>(wstring(L"assignment"), parent);;
    list<ESkoarToke::Kind>* desires = nullptr;

    if (++deep > 1000) {
        this->fail_too_deep();
    }

    // assignment -> AssOp settable
    desires = &skoarStats.desirables[94];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::AssOp] += 0.1f;
        noad->add_toke(L"Toke_AssOp", toker->burn(ESkoarToke::AssOp));
        this->settable(noad);
        --deep;
        return noad;
    }

    // Error State
    this->fail();
    return nullptr;
}

SkoarNoad* SkoarParser::math(SkoarNoad* parent) {
    SkoarNoad* noad = SkoarNoad::New<ESkoarNoad::math>(wstring(L"math"), parent);;
    list<ESkoarToke::Kind>* desires = nullptr;

    if (++deep > 1000) {
        this->fail_too_deep();
    }

    // math -> MathOp msgable
    desires = &skoarStats.desirables[95];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::MathOp] += 0.1f;
        noad->add_toke(L"Toke_MathOp", toker->burn(ESkoarToke::MathOp));
        noad->add_noad(this->msgable(noad));
        --deep;
        return noad;
    }

    // Error State
    this->fail();
    return nullptr;
}

SkoarNoad* SkoarParser::boolean(SkoarNoad* parent) {
    SkoarNoad* noad = SkoarNoad::New<ESkoarNoad::boolean>(wstring(L"boolean"), parent);;
    list<ESkoarToke::Kind>* desires = nullptr;

    if (++deep > 1000) {
        this->fail_too_deep();
    }

    // boolean -> BooleanOp expr
    desires = &skoarStats.desirables[96];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::BooleanOp] += 0.1f;
        noad->add_toke(L"Toke_BooleanOp", toker->burn(ESkoarToke::BooleanOp));
        noad->add_noad(this->expr(noad));
        --deep;
        return noad;
    }

    // Error State
    this->fail();
    return nullptr;
}

SkoarNoad* SkoarParser::times(SkoarNoad* parent) {
    SkoarNoad* noad = SkoarNoad::New<ESkoarNoad::times>(wstring(L"times"), parent);;
    list<ESkoarToke::Kind>* desires = nullptr;

    if (++deep > 1000) {
        this->fail_too_deep();
    }

    // times -> Times
    desires = &skoarStats.desirables[97];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::Times] += 0.1f;
        noad->add_toke(L"Toke_Times", toker->burn(ESkoarToke::Times));
        --deep;
        return noad;
    }

    // Error State
    this->fail();
    return nullptr;
}

SkoarNoad* SkoarParser::settable(SkoarNoad* parent) {
    SkoarNoad* noad = parent;
    list<ESkoarToke::Kind>* desires = nullptr;

    if (++deep > 1000) {
        this->fail_too_deep();
    }

    // settable -> Caesura
    desires = &skoarStats.desirables[98];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::Caesura] += 0.1f;
        noad->add_toke(L"Toke_Caesura", toker->burn(ESkoarToke::Caesura));
        --deep;
        return noad;
    }

    // settable -> Symbol
    desires = &skoarStats.desirables[99];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::Symbol] += 0.1f;
        noad->add_toke(L"Toke_Symbol", toker->burn(ESkoarToke::Symbol));
        --deep;
        return noad;
    }

    // settable -> listy
    desires = &skoarStats.desirables[100];
    if (toker->sees(desires) != nullptr) {
        noad->add_noad(this->listy(noad));
        --deep;
        return noad;
    }

    // settable -> Quarters
    desires = &skoarStats.desirables[101];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::Quarters] += 0.1f;
        noad->add_toke(L"Toke_Quarters", toker->burn(ESkoarToke::Quarters));
        --deep;
        return noad;
    }

    // settable -> Eighths
    desires = &skoarStats.desirables[102];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::Eighths] += 0.1f;
        noad->add_toke(L"Toke_Eighths", toker->burn(ESkoarToke::Eighths));
        --deep;
        return noad;
    }

    // settable -> Fairy
    desires = &skoarStats.desirables[103];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::Fairy] += 0.1f;
        noad->add_toke(L"Toke_Fairy", toker->burn(ESkoarToke::Fairy));
        --deep;
        return noad;
    }

    // Error State
    this->fail();
    return nullptr;
}

SkoarNoad* SkoarParser::msg_chain_node(SkoarNoad* parent) {
    SkoarNoad* noad = parent;
    list<ESkoarToke::Kind>* desires = nullptr;

    if (++deep > 1000) {
        this->fail_too_deep();
    }

    // msg_chain_node -> MsgOp msg msg_chain_node
    desires = &skoarStats.desirables[104];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::MsgOp] += 0.1f;
        noad->add_toke(L"Toke_MsgOp", toker->burn(ESkoarToke::MsgOp));
        noad->add_noad(this->msg(noad));
        this->msg_chain_node(noad);
        --deep;
        return noad;
    }

    // <e>
    --deep;
    return noad;
}

SkoarNoad* SkoarParser::msg(SkoarNoad* parent) {
    SkoarNoad* noad = SkoarNoad::New<ESkoarNoad::msg>(wstring(L"msg"), parent);;
    list<ESkoarToke::Kind>* desires = nullptr;

    if (++deep > 1000) {
        this->fail_too_deep();
    }

    // msg -> MsgNameWithArgs listy_suffix
    desires = &skoarStats.desirables[105];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::MsgNameWithArgs] += 0.1f;
        noad->add_toke(L"Toke_MsgNameWithArgs", toker->burn(ESkoarToke::MsgNameWithArgs));
        this->listy_suffix(noad);
        --deep;
        return noad;
    }

    // msg -> MsgName
    desires = &skoarStats.desirables[106];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::MsgName] += 0.1f;
        noad->add_toke(L"Toke_MsgName", toker->burn(ESkoarToke::MsgName));
        --deep;
        return noad;
    }

    // msg -> listy
    desires = &skoarStats.desirables[107];
    if (toker->sees(desires) != nullptr) {
        noad->add_noad(this->listy(noad));
        --deep;
        return noad;
    }

    // msg -> loop
    desires = &skoarStats.desirables[108];
    if (toker->sees(desires) != nullptr) {
        noad->add_noad(this->loop(noad));
        --deep;
        return noad;
    }

    // Error State
    this->fail();
    return nullptr;
}

SkoarNoad* SkoarParser::cthulhu_prime(SkoarNoad* parent) {
    SkoarNoad* noad = parent;
    list<ESkoarToke::Kind>* desires = nullptr;

    if (++deep > 1000) {
        this->fail_too_deep();
    }

    // cthulhu_prime -> expr Semicolon RWing
    desires = &skoarStats.desirables[109];
    if (toker->sees(desires) != nullptr) {
        noad->add_noad(this->expr(noad));
        skoarStats.tokeFreq[ESkoarToke::Semicolon] += 0.1f;
        noad->add_toke(L"Toke_Semicolon", toker->burn(ESkoarToke::Semicolon));
        skoarStats.tokeFreq[ESkoarToke::RWing] += 0.1f;
        noad->add_toke(L"Toke_RWing", toker->burn(ESkoarToke::RWing));
        --deep;
        return noad;
    }

    // cthulhu_prime -> Nosey Semicolon RWing
    desires = &skoarStats.desirables[110];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::Nosey] += 0.1f;
        noad->add_toke(L"Toke_Nosey", toker->burn(ESkoarToke::Nosey));
        skoarStats.tokeFreq[ESkoarToke::Semicolon] += 0.1f;
        noad->add_toke(L"Toke_Semicolon", toker->burn(ESkoarToke::Semicolon));
        skoarStats.tokeFreq[ESkoarToke::RWing] += 0.1f;
        noad->add_toke(L"Toke_RWing", toker->burn(ESkoarToke::RWing));
        --deep;
        return noad;
    }

    // Error State
    this->fail();
    return nullptr;
}

SkoarNoad* SkoarParser::cond_ifs(SkoarNoad* parent) {
    SkoarNoad* noad = parent;
    list<ESkoarToke::Kind>* desires = nullptr;

    if (++deep > 1000) {
        this->fail_too_deep();
    }

    // cond_ifs -> cond_if cond_ifs_suffix
    desires = &skoarStats.desirables[111];
    if (toker->sees(desires) != nullptr) {
        noad->add_noad(this->cond_if(noad));
        this->cond_ifs_suffix(noad);
        --deep;
        return noad;
    }

    // Error State
    this->fail();
    return nullptr;
}

SkoarNoad* SkoarParser::cond_if(SkoarNoad* parent) {
    SkoarNoad* noad = SkoarNoad::New<ESkoarNoad::cond_if>(wstring(L"cond_if"), parent);;
    list<ESkoarToke::Kind>* desires = nullptr;

    if (++deep > 1000) {
        this->fail_too_deep();
    }

    // cond_if -> optional_voice expr CondIf if_body cond_else
    desires = &skoarStats.desirables[112];
    if (toker->sees(desires) != nullptr) {
        this->optional_voice(noad);
        noad->add_noad(this->expr(noad));
        skoarStats.tokeFreq[ESkoarToke::CondIf] += 0.1f;
        noad->add_toke(L"Toke_CondIf", toker->burn(ESkoarToke::CondIf));
        noad->add_noad(this->if_body(noad));
        this->cond_else(noad);
        --deep;
        return noad;
    }

    // Error State
    this->fail();
    return nullptr;
}

SkoarNoad* SkoarParser::cond_ifs_suffix(SkoarNoad* parent) {
    SkoarNoad* noad = parent;
    list<ESkoarToke::Kind>* desires = nullptr;

    if (++deep > 1000) {
        this->fail_too_deep();
    }

    // cond_ifs_suffix -> Newline cond_ifs
    desires = &skoarStats.desirables[113];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::Newline] += 0.1f;
        noad->add_toke(L"Toke_Newline", toker->burn(ESkoarToke::Newline));
        this->cond_ifs(noad);
        --deep;
        return noad;
    }

    // <e>
    --deep;
    return noad;
}

SkoarNoad* SkoarParser::if_body(SkoarNoad* parent) {
    SkoarNoad* noad = SkoarNoad::New<ESkoarNoad::if_body>(wstring(L"if_body"), parent);;
    list<ESkoarToke::Kind>* desires = nullptr;

    if (++deep > 1000) {
        this->fail_too_deep();
    }

    // if_body -> phrases
    desires = &skoarStats.desirables[114];
    if (toker->sees(desires) != nullptr) {
        this->phrases(noad);
        --deep;
        return noad;
    }

    // Error State
    this->fail();
    return nullptr;
}

SkoarNoad* SkoarParser::cond_else(SkoarNoad* parent) {
    SkoarNoad* noad = parent;
    list<ESkoarToke::Kind>* desires = nullptr;

    if (++deep > 1000) {
        this->fail_too_deep();
    }

    // cond_else -> CondIf if_body
    desires = &skoarStats.desirables[115];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::CondIf] += 0.1f;
        noad->add_toke(L"Toke_CondIf", toker->burn(ESkoarToke::CondIf));
        noad->add_noad(this->if_body(noad));
        --deep;
        return noad;
    }

    // <e>
    --deep;
    return noad;
}

SkoarNoad* SkoarParser::loop_body(SkoarNoad* parent) {
    SkoarNoad* noad = SkoarNoad::New<ESkoarNoad::loop_body>(wstring(L"loop_body"), parent);;
    list<ESkoarToke::Kind>* desires = nullptr;

    if (++deep > 1000) {
        this->fail_too_deep();
    }

    // loop_body -> phrases
    desires = &skoarStats.desirables[116];
    if (toker->sees(desires) != nullptr) {
        this->phrases(noad);
        --deep;
        return noad;
    }

    // Error State
    this->fail();
    return nullptr;
}

SkoarNoad* SkoarParser::loop_condition(SkoarNoad* parent) {
    SkoarNoad* noad = SkoarNoad::New<ESkoarNoad::loop_condition>(wstring(L"loop_condition"), parent);;
    list<ESkoarToke::Kind>* desires = nullptr;

    if (++deep > 1000) {
        this->fail_too_deep();
    }

    // loop_condition -> LoopSep expr
    desires = &skoarStats.desirables[117];
    if (toker->sees(desires) != nullptr) {
        skoarStats.tokeFreq[ESkoarToke::LoopSep] += 0.1f;
        noad->add_toke(L"Toke_LoopSep", toker->burn(ESkoarToke::LoopSep));
        noad->add_noad(this->expr(noad));
        --deep;
        return noad;
    }

    // <e>
    --deep;
    return noad;
}


void SkoarParser::sortDesirables() {
    localSortDesirables();
}

